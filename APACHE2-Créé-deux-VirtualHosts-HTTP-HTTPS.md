...
> Ce guide explique comment configurer deux VirtualHosts sur Apache2 : un pour HTTP (port 80) et un pour HTTPS (port 443) avec certificat auto-signé.
...
- Prérequis
- Étape 1 : VirtualHost HTTP
- Étape 2 : Sécurisation Apache
- Étape 3 : Génération du certificat SSL
- Étape 4 : Installation du certificat
- Étape 5 : VirtualHost HTTPS
- Étape 6 : Redirection HTTP → HTTPS
- Étape 7 : Activation & rechargement Apache
...
# Date de création : le 30-07-2025
# Date de modification : le 30-07-2025
# default-ssl.conf - Exemple concernant le VirtualHost default-ssl.conf
# /etc/apache2/sites-available/
# --------------------------------------------------------------------------
<VirtualHost *:443>
    ServerAdmin webmaster@localhost
...
Éditez `000-default.conf` :

```bash
nano /etc/apache2/sites-available/000-default.conf
```

Dans le bloc `<VirtualHost>`, ajoutez à la fin :
